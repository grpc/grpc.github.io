...
After having several discussions with gRPC-Java and gRPC-C core team we realized that gRPC-Go's connection-level flow control was overly restrictive in the sense that window updates on the connection depended upon if the application had read data from it or not. It must be noted that it makes perfect sense to have stream-level flow control depended on application read but not so much for connection-level flow control. The rationale is as floows: A connection is shared by several streams (RPCs). Now, if there were at least one stream that read slowly or didn't read at all, it would hamper the performance or completely stall other streams on that connection. This happens because we won't send out window updates on the connection until that slow or inactive stream read data. Therefore, it makes sense to decouple the connection's flow control from application reads. 

However, this begs at least two questions: 1. Won't a client be able to send as much data as it wants to the server by creating new streams when one runs out? 2. Why even have connection-level flow control if the stream-level flow control is enough.

The answer to the first question is short and simple: no. A server has an option to limit the number of streams that it intends to server concurrently. Therefore, although at first it may seem like a problem, it really is not.

The second question is very interesting: why even bother with connection-level flow control?

It is true that stream-level flow control is sufficient to throttle a sender from sending too much data. But not having connection-level flow control (or using an unlimited connection-level window) makes it so that when things get slower on a stream, opening a new one will appear to make things faster. This is a slippery slope and will only take one so far. However, having a connection-level flow control window set to the Bandwidth Delay Product (BDP) of the network puts an upper-bound on how much performance can realistically be squeezed out of the network.
...
During our implementation we experimented with different parameters like instead of putting our window size to twice the bdp sample we put it to 4/3 times the bdp sample. Note although this is more accurate (since the noted bdp was smaller than or equal to 1.5 the real bandwidth and ideally we want to be twice the real bandwidth so multiplying the sample with 4/3 should put us there), it's not the best option to choose since the growth is slow and it takes some tens of RPCs to ramp up.
...
We noticed a bug in our transport layer which causes us to make a flush syscall for every data frame we write, even if the same goroutine has more data to send. We can batch a lot of these writes to use only one flush. This in fact will not be a big change to the code itself. The performance gain noted by this change is: // numbers

In our efforts to get rid of unnecessary flushes we recently combined the headers and data write for unary and server streaming RPCs to one flush on the client-side. // PR

Another related idea proposed by one of our users (yay community!) was to combine a server response to a unary RPC into one flush. We are currently looking into that as well.


### Memory allocation

For every data frame read from the wire a new memory allocation takes place. The same holds true at the gRPC layer for every new message for decompressing and decoding. These allocations result in excessive garbage collection cycles, which are expensive, and can be reduced if we reuse memory buffers. We are working on an idea to create a cache of these allocated buffers and reuse them when possible. We realize that these buffers are different sizes and using the maximum size for all of them will result in wasting a lot of memory. Therefore, we intend to allocate and cache buffers of fixed sizes(tentatively powers of two). So now when reading x bytes from the wire we can find the nearest power of 2 greater than x and reuse a buffer from our cache if available or allocate a new one if need be. We will be using golang sync Pools so we don't have to worry about garbage collection. However, we will need to run sufficient tests before committing to this.

The same concept can be used at the gRPC layer; admittedly the sizes there are bigger but easily capped by 2^40 (so a backing array of size 40 should suffice). However, I must admit we haven't tested this cache on the gRPC layer and thus can't guarantee if it really will work.
